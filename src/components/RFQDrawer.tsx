...
const RFQDrawer = ({ open, onClose, product, grade, category }: RFQDrawerProps) => {
  ...
  const handleSubmit = async () => {
    ...
    required.forEach((id) => {
      ...
      if (!values[id]) newErrors.add(id);
    });

    // Also grab optional fields
    const gstEl = form.querySelector("#gstNum") as HTMLInputElement;
    const freqEl = form.querySelector("#frequency") as HTMLSelectElement;
    const addlEl = form.querySelector("#addlReqs") as HTMLTextAreaElement;
    const desigEl = form.querySelector("#designation") as HTMLInputElement;
    ...
    try {
      const message = [
        ...
        gstEl?.value ? `GST: ${gstEl.value}` : "",
        `Industry: ${values.industrySeg}`,
        `Grade: ${values.gradeReq}`,
        `Quantity: ${values.quantity} MT`,
        freqEl?.value ? `Frequency: ${freqEl.value}` : "",
        `Delivery: ${values.deliveryLoc}`,
        `Urgency: ${urgency}`,
        desigEl?.value ? `Designation: ${desigEl.value}` : "",
        addlEl?.value ? `Notes: ${addlEl.value}` : "",
      ].filter(Boolean).join("\n");
      ...
      const { error } = await supabase.from("enquiries").insert({
        ...
        company: values.companyName,
        message,
        product_interest: `${product} - ${values.gradeReq}`,
        source_type: "rfq",
      });
    ...
    <>
      ...
      >
        <div className="bg-primary text-primary-foreground px-7 py-5 flex justify-between items-start flex-shrink-0">
          <div>
            <h3 className="text-lg font-bold flex items-center gap-2"><FileText className="w-5 h-5" /> Request for Quotation</h3>
            <p className="text-sm opacity-75 mt-0.5">Our team will respond within 2 business hours</p>
          </div>
          <button onClick={handleClose} className="bg-primary-foreground/15 hover:bg-primary-foreground/28 rounded-md w-8 h-8 flex items-center justify-center transition-colors"><X className="w-4 h-4" /></button>
        </div>

        <div className="bg-secondary border-b border-brand-gray-200 px-7 py-3.5 flex items-center gap-3 flex-shrink-0">
          <div className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center"><CatIcon className="w-5 h-5 text-primary" /></div>
        ...
        {!submitted ? (
          <>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    ...
                    <select id="industrySeg" className={inputClass("industrySeg")} onInput={() => setErrors((prev) => { const n = new Set(prev); n.delete("industrySeg"); return n; })}>
                      ...
                      {["Packaging", "Automotive", "Pipes & Fittings", "Textiles / Apparel", "FMCG / Pharma", "Construction", "Agriculture", "Electrical / Cables", "Other"].map((o) => (<option key={o}>{o}</option>))}
                    </select>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <select id="frequency" className={inputClass("frequency")}>
                      ...
                      {["One-Time", "Monthly", "Quarterly", "Spot Purchase"].map((o) => (<option key={o}>{o}</option>))}
                    </select>
                  </div>
                </div>
                <div className="mb-3">
                ...
                <div className="mb-3">
                  ...
                  <div className="flex gap-2 flex-wrap mt-1">
                    {["Immediate", "Within 1 Week", "1 – 4 Weeks", "Flexible"].map((u) => (
                      <button key={u} type="button" onClick={() => setUrgency(u)} className={`px-3.5 py-2 rounded-md border text-xs font-semibold transition-all ${urgency === u ? "bg-primary text-primary-foreground border-primary" : "bg-background text-brand-gray-700 border-brand-gray-200 hover:border-primary"}`}>{u}</button>
                    ))}
                  </div>
                </div>
            ...
            <div className="border-t border-brand-gray-200 px-7 py-5 bg-brand-gray-50 flex-shrink-0">
              <button onClick={handleSubmit} disabled={saving} className="w-full bg-destructive text-destructive-foreground font-bold py-3.5 rounded-lg hover:bg-brand-gold-dark transition-colors flex items-center justify-center gap-2 text-base disabled:opacity-50">
                <FileText className="w-4 h-4" /> {saving ? "Submitting…" : "Submit Quotation Request"}
              </button>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center px-8 gap-4">
            <div className="w-[72px] h-[72px] rounded-full bg-green-100 flex items-center justify-center"><CheckCircle2 className="w-8 h-8 text-green-600" /></div>
            <h3 className="text-xl font-extrabold text-foreground">Enquiry Submitted!</h3>
            <p className="text-sm text-muted-foreground max-w-xs leading-relaxed">Your quotation request has been received. Our team will contact you within <strong>2 business hours</strong> with pricing and availability.</p>
            <div className="bg-secondary border border-dashed border-primary px-5 py-2.5 rounded-lg text-sm font-bold text-primary">RFQ Ref: {refNo}</div>
            <button onClick={handleClose} className="bg-destructive text-destructive-foreground font-bold px-6 py-3 rounded-lg mt-2 hover:bg-brand-gold-dark transition-colors">Close</button>
          </div>
        )}
      </div>
    </>
