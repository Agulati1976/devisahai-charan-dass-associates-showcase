...
import { MessageSquare, Eye, X, Mail, Phone, Building2, Package } from "lucide-react";
    ...
    <div>
        ...
        <div className="flex gap-2 flex-wrap">
          {["all", "new", "contacted", "converted", "closed"].map((s) => (
            <button
              key={s}
              onClick={() => setFilter(s)}
              className={`px-3 py-1.5 text-xs font-bold rounded-lg capitalize transition-colors ${filter === s ? "bg-primary text-primary-foreground" : "bg-brand-gray-100 text-muted-foreground hover:bg-brand-gray-200"}`}
            >
              {s}
            </button>
          ))}
      ...
      <div className="bg-card border border-brand-gray-200 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {enquiries.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-12 text-muted-foreground">No enquiries found.</td></tr>
            ) : enquiries.map((e) => (
              <tr key={e.id} className="border-b border-brand-gray-100 hover:bg-brand-gray-50/50">
                ...
                <td className="px-4 py-3 text-muted-foreground hidden lg:table-cell">{format(new Date(e.created_at), "dd MMM yyyy")}</td>
      ...
      {selected && (
        <div className="fixed inset-0 bg-foreground/40 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-brand-gray-200 rounded-2xl w-full max-w-lg">
            <div className="flex items-center justify-between px-6 py-4 border-b border-brand-gray-200">
              <h2 className="text-lg font-extrabold text-foreground">Enquiry Details</h2>
                ...
                <div className="flex gap-2 flex-wrap">
                  {["new", "contacted", "converted", "closed"].map((s) => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-bold rounded-lg capitalize transition-colors ${selected.status === s ? "bg-primary text-primary-foreground" : "bg-brand-gray-100 text-muted-foreground hover:bg-brand-gray-200"}`}
                    >
                      {s}
                    </button>
                  ))}
            ...
            <div className="px-6 py-4 border-t border-brand-gray-200 flex justify-between">
              <button onClick={() => handleDelete(selected.id)} className="text-red-500 text-sm font-semibold hover:text-red-700">Delete Enquiry</button>
