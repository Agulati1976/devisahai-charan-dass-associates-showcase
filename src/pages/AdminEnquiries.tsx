      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["all", "new", "contacted", "converted", "closed"].map((s) => (
          <button
            ...
            className={`px-3 py-1.5 text-xs font-bold rounded-lg capitalize transition-colors ${statusFilter === s ? "bg-accent text-accent-foreground" : "bg-brand-gray-100 text-muted-foreground hover:bg-brand-gray-200"}`}
          >
            {s}
          </button>
        ))}
        ...
        <table className="w-full text-sm">
          <thead className="bg-brand-gray-50 border-b border-brand-gray-200">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-semibold text-muted-foreground hidden md:table-cell">Product</th>
          ...
          <tbody>
            {enquiries.length === 0 ? (
              ...
              <tr key={e.id} className="border-b border-brand-gray-100 hover:bg-brand-gray-50/50">
                ...
                <td className="px-4 py-3 text-muted-foreground text-xs hidden md:table-cell">{e.product_interest || "—"}</td>
                <td className="px-4 py-3 hidden md:table-cell">
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="flex items-center gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground capitalize">{selected.source_type === "rfq" ? "RFQ Submission" : "Contact Form"} • {format(new Date(selected.created_at), "dd MMM yyyy, hh:mm a")}</p>
                </div>
              </div>
